...
/**
 * Check if the current token is for an admin user
 * 
 * @returns True if the current user is authenticated as an admin
 */
export function isAdminToken(): boolean {
  ...
  return localStorage.getItem(AUTH_STORAGE_KEYS.IS_ADMIN) === 'true';
}

/**
 * Clear all auth tokens and related storage
 */
export function clearAuthTokens(): void {
  localStorage.removeItem(AUTH_STORAGE_KEYS.AUTH_TOKEN);
  localStorage.removeItem(AUTH_STORAGE_KEYS.IS_ADMIN);
